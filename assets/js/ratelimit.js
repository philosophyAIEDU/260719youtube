 ...
 *
 * 두 개의 독립된 한도를 둡니다:
 *   - 기본(analyze/chat)  : PhilApp.ratelimit.check()/record()/counts()
 *   - 자막(transcript)    : PhilApp.ratelimit.transcript.check()/record()/counts()
 *     (captions API는 훨씬 비싸서 별도의, 더 엄격한 한도로 보호합니다)
 * ===================================================================== */
...
PhilApp.ratelimit = (function () {
  ...
  function makeLimiter(storageKey, getLimits) {
    function load() {
    ...
    function check() {
      ...
      if (c.day >= lim.perDay) {
        return {
          allowed: false,
          message: "오늘 이 브라우저의 무료 사용 한도(" + lim.perDay +
      ...
      if (c.hour >= lim.perHour) {
        ...
        return {
          allowed: false,
          message: "잠시 후 다시 시도해 주세요. 시간당 사용 한도(" + lim.perHour +
  ...
  var main = makeLimiter("phil_usage_log", function () { return cfg.RATE_LIMIT || { perHour: 8, perDay: 25 }; });
  var transcript = makeLimiter("phil_transcript_usage_log", function () { return cfg.TRANSCRIPT_RATE_LIMIT || { perHour: 1, perDay: 3 }; });

  return {
    check: main.check, record: main.record, counts: main.counts,
